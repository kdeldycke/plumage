...
- Serve Bootstrap Icons from the site rather than from cdnjs, and move its version into `plumage/package.json`, where Dependabot reaches it under the same cooldown as the rest of the pipeline. It had sat at `1.11.3` since nothing automated the pin; the font is now `1.13.1`. A new `sync-bootstrap-icons` job copies the `woff2` out of the pinned release, the way `sync-pygments-styles` regenerates the stylesheets. Only `woff2` ships, which every browser the theme targets reads.
